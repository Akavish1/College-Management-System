...
namespace merge_B
{
    public partial class Schedule : MetroForm
    {
        ...
        private double getcumCredits()
        {
            double result = 0;
            //do calc
            if(Int32.Parse((Globals.LoggedUser as Classes.Student).getSemester())==1)
            {
                result = 0.0;
            }
            else
        ...
        {
            CourseDetails cd = new CourseDetails(coursename);
            cd.ShowDialog();
        }
        ...
        {
            Application.Run(new CourseDetails(index));
        }
        ...
        private void compCoursesText_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //Do registering.. and save to DB. ==> #3
                merge_B.Forms.Student.RegisterToCourse a = new merge_B.Forms.Student.RegisterToCourse(compCoursesText.GetItemText(compCoursesText.SelectedItem));
                a.ShowDialog();
        ...
        {
            merge_B.Forms.Student.SearchCourse a=new merge_B.Forms.Student.SearchCourse();
            a.ShowDialog();
        }
        ...
        private void OptionCourses_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (OptionCourses.SelectedItem == null)
                return;
            Forms.Student.RegisterToCourse Reg = new Forms.Student.RegisterToCourse(OptionCourses.GetItemText(OptionCourses.SelectedItem).ToString());
            Reg.ShowDialog();
        }
