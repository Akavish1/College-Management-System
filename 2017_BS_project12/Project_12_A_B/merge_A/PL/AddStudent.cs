    ...
    public partial class AddStudentform : Form
    {
        ...
        private void Save_Button_Click(object sender, EventArgs e)
        {
            ...
            if (EditStud == false) // Adding a new student to database
            {
                if (countAVG == true && countName == true && Academic_comboBox1.Text != ""&& Semester_CB.Text!="" && countEmail == true && coutADS == true && countAVG == true)
                {
                    ...
                    {
                        con.Open();
                        SqlCommand com = new SqlCommand("INSERT INTO Project_Sce.CodeLayer.Student(StudID,Name,Year,Average,Address,Email,Semester) VALUES(@id,@name,@year,@ave,@address,@email,@semester)", con);
        ...
        private void Populate()
        {
            ...
            try
            {
                ...
                while (read.Read())
                {
                    if (Project_Sce.CodeLayer.StudentID.ToString().Trim() == read["StudID"].ToString().Trim())
